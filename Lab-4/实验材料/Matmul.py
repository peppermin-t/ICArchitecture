...
class Matmul(object):
    ...
    def send_data(self, data, block_name, offset='default'):
        ...
        '''
        self.bram.write(data, block_name, offset)
    ...
    def send_instr(self, m, p, n):
        ...
        p_hex = p << 32
        n_hex = n << 16
        instr = (m_hex + p_hex + n_hex).to_bytes(8, byteorder='big')
        self.bram.write(instr, 'ir', offset='instr')

    def send_flag(self):
        '''发送flag=1信号'''
        # 向ir块中写入flag信息
        flag_01 = b"\x01\x00\x00\x00"   # 最左侧字节存储在低地址处，往右依次存储在更高地址
        self.bram.write(flag_01, 'ir', offset='flag')
    ...
    def recv_output(self, output_shape: tuple):
        ...
        '''
        output_arr = self.bram.read(output_shape[0] * output_shape[1], 'output', dtype=np.int8).reshape(output_shape)
        return output_arr
